...
function routeFor(file){const rel=path.relative(ROOT,file).replaceAll('\\','/');if(rel==='index.html')return '/';if(rel==='404.html')return null;return '/'+rel.replace(/index\.html$/,'')}
const routes=[...new Set(walk(ROOT).map(routeFor).filter(Boolean))].sort();
